...
async def generate_prompt(req: Request):
    try:
        ...
        if image_url and isinstance(image_url, str):
            ...
            # 加入完整檢查與 debug log
            image_path = os.path.join(UPLOAD_DIR, image_filename)
            print(f"[INFO] 解析後的 filename: {image_filename}")
            print(f"[INFO] 檢查圖片文件: {image_path}, 存在: {os.path.exists(image_path)}")

            if os.path.exists(image_path):
                data["image_url"] = f"https://epson-hey-echo.onrender.com/view-image/{image_filename}"
                print(f"[INFO] 更新 image_url 為: {data['image_url']}")
            else:
